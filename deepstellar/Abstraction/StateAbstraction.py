...
class StateAbstraction:
    ...
    def pca_fit(self):
        """
        Read svhn_data from the data_repo and calculate the first comp_num principal components.
        For choose to sample the svhn_data before fitting PCA model
        """

        # read from svhn_data repo
        all_sample_data = []
    ...
    def pca_trans(self):
        """
        Transform all the svhn_data with the PCA model and save the transformed svhn_data to pca_trans folder inside the repo folder
        """
    ...
    def get_quantization_matrix(self):
        """
        Read the PCA-transformed svhn_data, and calculate the auxiliary matrix for quantization
        """
        fit_data = self.get_pca_trans_data()
        fit_data = np.array([s for seq in fit_data for s in seq])
        print('fit svhn_data shape:')
