...
class PrioritySelectStrategy(AbsSelectStrategy):
    ...
    # no target_size
    def get_max_coverage_sequence(self, Tx, Ty, n, M, dense_model, base_path=None, use_add=False, is_save_ps=False, prefix="",
        ...
        for i in range(n):
            csv_data["label"] = i
            ''' BiLSTM模型对SVHN数据集的预测标签比较单一，导致没有出现某类别的数据，就会报错Tx_i为空 '''
            Tx_i, Ty_i, T_idx_arr = get_data_by_label_with_idx(Tx, Ty, i)
            # 对于图像数据进行预处理
            # 对应RNNModels中对于RNN模型的训练数据预处理input_preprocess函数
            ...
            # 如果该预测标签没有数据，就跳到下一预测类别
            if Tx_i.shape[0] == 0:
                continue

            lstm_out = M.predict(Tx_i, verbose=0)[1]
            Tx_prob_matrixc = np.zeros((lstm_out.shape[0], dense_model.predict(lstm_out[:, 0, :], verbose=0).shape[1]))
            weight_sum = 0
            for t in range(lstm_out.shape[1] - 1):
                # weight = (t+1) ** 0.1
                ''' 论文和一作说ATS和DeepGini都是用(t+1)平方的时间权重 '''
                weight = (t+1) ** 2
                weight_sum += weight
                dense_array = lambda x: x[:, t, :]
                tmp = dense_model.predict(np.array(dense_array(lstm_out)), verbose=0)
