...
class UniformSelectStrategy(AbsSelectStrategy):
    ...
    def datasets_select_detail(self, symbol, S_mid, n, i, target_size, idx_mid, base_path=None, is_analyze=False):
        ...
        if len(idx_mid) != len(C_Xr_list):
            raise ValueError("len idx not eq len svhn_data")
            ### step1
        all_rank_arr = []
        ...
        while len(rel_mid_idx) < target_size:
            ...
            temp_c_arr.append(c)
            # print("# current no select svhn_data point", len(X_no_i))
            ...
            if max_s_i != 0:
                ...
                if len(X_no_i) != len(S_mid):
                    raise ValueError("no point left but X_no_i does not hav all svhn_data points")
        ...
        if len(Xr_select_i) != target_size:
            raise ValueError("svhn_data points size not eq target_size")
        if len(Xr_select_i) != len(set(Xr_select_i)):
            raise ValueError("some svhn_data points  repeatly select")
...
class select_status(object):
    ...
    def __init__(self) -> None:
        self.SELECT_ALL_LABLES = 0  # all svhn_data
        self.SELECT_ZERO_LABLES = 1  # all svhn_data  but Smid no svhn_data
        self.SELECT_ALL_MID_LABLES = 2  # Smid all svhn_data sleceted
        self.SELECT_ALL_MID_LABLES_WITH_HIGH = 3  # extend svhn_data with high coff
        self.SELECT_MID_LABLES_CAM = 4  # svhn_data not enough use cam
        self.SELECT_MID_LABLES_CAM_ALL = 5  # svhn_data get max cov
        self.SELECT_MID_LABLES_CAM_CTM = 6  # svhn_data not enough use cam and ctm
